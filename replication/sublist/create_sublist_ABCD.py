...
parser = argparse.ArgumentParser(description='generate a sublist of ABCD subjects with RS data and which passed QC',
                                 formatter_class=lambda prog: argparse.ArgumentDefaultsHelpFormatter(prog, width=100))
parser.add_argument('dataset_dir', type=str, help=('path to existing installation of ABCD dataset from' +
                                                   'https://collection3165.readthedocs.io/'))
parser.add_argument('output', type=str, help='name for output sublist file')
parser.add_argument('--source', dest='source', type=str, default=None, help='source to use for datalad get')
parser.add_argument('--log', dest='log', type=str, default=None, help='log file to write exclusion causes to')
...
scanner_info = pd.read_table(scanner_info_file, delim_whitespace=True, usecols=['subjectkey', 'mri_info_manufacturer'],
                             dtype=str)
...
sublist_start = [f'NDAR_{item.lstrip("sub-NDAR")}' for item in listdir(deriv_dir) if 'sub-NDARINV' in item]
sublist = []
for subject in sublist_start:
    ...
    censor_file = f'sub-NDAR{subject.lstrip("NDAR_")}_ses-baselineYear1Arm1_task-rest_desc-filtered_motion_mask.mat'
    ...
    if path.islink(path.join(func_dir, censor_file)):
        ...
        if remain_seconds/60 < 4:
            print(f'{subject} has {remain_seconds/60} minutes of RS data left after censoring', file=log)
            continue
        ...
        for run in range(int(total_frames/run_length)):
            ...
            if remaining_frames/383 < 0.5:
                print(f'{subject} has {remaining_frames} frames left for run {run} after censoring', file=log)
                continue
            else:
