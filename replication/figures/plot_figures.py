...
features = [
    "rest static FC", "rest dynamic FC", "rest EC", "rest network statisics", "rest gradients",
    ...
    "MOTOR EC", "WM EC", "RELATIONAL EC", "SOCIAL EC", "myelin estimate", "GMV", "CS", "CT",
    "GMV Connectivity", "CS Connectivity", "CT Connectivity", "SC (count)", "SC (length)", "FA",
    "MD", "AD", "RD", "confound"]
targets = [
...
for acc_type in ["r", "cod"]:
    ...
    if (not if_acc_file.exists()) or args.overwrite:
        ...
        sns.relplot(
            ...
            x="Number of features", y="Accuracy", hue="Target", col="Dataset", palette=cmap,
            marker="o", errorbar=("ci", 95), height=15, aspect=0.4,
...
for acc_type in ["r", "cod"]:
    ...
    if (not nec_acc_file.exists()) or args.overwrite:
        ...
        g = sns.catplot(
            ...
            x="Accuracy", hue="Target", col="Dataset", orient="h", palette=cmap, height=10,
            aspect=0.5, showfliers=False, legend=False, sharey=True, sharex=False)
...
if (not nec_features_file.exists()) or args.overwrite:
    ...
    sns.displot(
        data=nec_sorted, kind="hist", y="Feature", hue="Target", col="Dataset", stat="percent",
        hue_order=targets, palette=cmap, multiple="stack", height=15, aspect=0.4,
        facet_kws={"legend_out": True, "sharey": True})
